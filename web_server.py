...
<style>
:root{--ink:#090909;--paper:#f7f7f5;--muted:#747474;--line:#0a0a0a;--yellow:#ffd63d;--green:#52d878;--red:#ff5d5d;--gray:#e9e9e7;--shadow:5px 5px 0 var(--ink);--font:Arial,Helvetica,sans-serif;--mono:"Courier New",monospace}.dark{--ink:#f5f5f2;--paper:#101010;--muted:#a5a5a0;--line:#f5f5f2;--gray:#242424}.dark body,.dark .shell{background:var(--paper);color:var(--ink)}.dark .mast{background:#171717}.sidebar{position:fixed;z-index:5;left:0;top:0;bottom:0;width:226px;background:var(--ink);color:var(--paper);padding:28px 18px;display:flex;flex-direction:column;gap:26px}.sidebar-brand{font-weight:1000;font-size:21px;letter-spacing:.08em;border-bottom:2px solid var(--paper);padding:0 0 22px}.sidebar-brand small{display:block;color:var(--green);font:11px var(--mono);margin-top:8px}.side-nav{display:flex;flex-direction:column;gap:8px}.side-nav button{border:2px solid transparent;background:transparent;color:var(--paper);padding:13px 12px;text-align:left;font-weight:900;letter-spacing:.08em}.side-nav button:hover,.side-nav button.active{border-color:var(--paper);background:var(--paper);color:var(--ink)}.side-status{margin-top:auto;border-top:2px solid var(--paper);padding-top:18px;font:11px/1.7 var(--mono);color:#bbb}.side-status strong{color:var(--green);display:block}.shell{max-width:none;margin:0 0 0 226px;min-height:100vh;border-left:3px solid var(--line);border-right:0;background:var(--paper)}.alert,.mast,.wrap,.footer{padding-left:48px;padding-right:48px}.mast{padding-top:48px}.footer{margin-left:0}
.chat-panel{max-width:980px;margin:0 auto;border:3px solid var(--line);background:#fff;box-shadow:5px 5px 0 var(--ink)}.chat-header{display:flex;justify-content:space-between;align-items:center;gap:18px;padding:22px 24px;border-bottom:3px solid var(--line);background:#fafaf9}.chat-title{font-size:24px;font-weight:1000;letter-spacing:.02em}.chat-subtitle{display:block;color:var(--muted);font:12px var(--mono);margin-top:7px}.chat-meta{display:flex;gap:8px;align-items:center;flex-wrap:wrap;justify-content:flex-end}.chat-meta select{min-height:42px;width:190px;font-size:13px;padding:8px 12px;box-shadow:2px 2px 0 var(--ink)}.chat-messages{min-height:430px;max-height:55vh;overflow:auto;padding:28px;background:#f2f2f0;display:flex;flex-direction:column;gap:18px}.message{max-width:82%;border:3px solid var(--line);padding:16px 18px;box-shadow:3px 3px 0 var(--ink);font:15px/1.55 var(--font);letter-spacing:.02em;white-space:pre-wrap}.message.system{align-self:flex-start;background:#fff}.message.user{align-self:flex-end;background:var(--ink);color:#fff}.message.agent{align-self:flex-start;background:var(--green)}.message-label{display:block;font:10px var(--mono);font-weight:900;letter-spacing:.12em;margin-bottom:8px;opacity:.7}.composer{padding:20px 24px;border-top:3px solid var(--line);background:#fff}.composer-row{display:flex;gap:12px;align-items:stretch}.composer textarea{min-height:62px;resize:vertical;box-shadow:3px 3px 0 var(--ink)}.send-btn{min-width:132px}.quick-prompts{display:flex;gap:8px;flex-wrap:wrap;margin-top:13px}.quick-prompts button{border:2px solid var(--line);background:var(--gray);padding:8px 10px;font:11px var(--mono);font-weight:700}.quick-prompts button:hover{background:var(--yellow)}.chat-footnote{font:11px var(--mono);color:var(--muted);margin-top:12px}.sidebar .side-nav button:first-child{background:var(--paper);color:var(--ink);border-color:var(--paper)}

body{background:#111;color:#f5f5f2}.shell{margin:0;max-width:none;min-height:100vh;background:#111;border:0}.alert,.mast,.footer{display:none}.sidebar{transform:translateX(-105%);transition:transform .22s ease;box-shadow:8px 0 30px #000}.sidebar.open{transform:translateX(0);z-index:30}.wrap{padding:20px 22px;min-height:100vh;display:flex;align-items:center;justify-content:center}.chat-panel{width:min(100%,980px);background:#181818;color:#f5f5f2;border:1px solid #454545;box-shadow:0 12px 40px #000}.chat-header{background:#181818;border-bottom:1px solid #454545;padding:18px 22px}.chat-title{font-size:19px}.chat-subtitle{color:#898989}.chat-messages{background:#111;min-height:58vh;max-height:68vh;padding:26px 22px}.message{border:1px solid #555;box-shadow:none}.message.system{background:#202020;color:#f5f5f2}.message.user{background:#373737;color:#f5f5f2}.message.agent{background:#202020;color:#f5f5f2;border-color:#555}.composer{background:#181818;border-top:1px solid #454545}.composer textarea{background:#202020;color:#f5f5f2;border:1px solid #555;box-shadow:none}.composer textarea::placeholder{color:#858585}.quick-prompts button{background:#202020;color:#aaa;border-color:#555}.chat-meta select{background:#202020;color:#f5f5f2;border-color:#555;box-shadow:none}.menu-toggle{border:1px solid #555;background:#202020;color:#fff;width:42px;height:38px;font-size:20px;margin-right:12px}.chat-title-row{display:flex;align-items:center}.lower{display:none!important}#runtime,#workspace{display:none!important}.chat-footnote{color:#777}.badge{box-shadow:none;border:1px solid #111;font-size:10px;padding:6px 8px}@media(max-width:720px){.wrap{padding:0;display:block}.chat-panel{width:100%;min-height:100vh;border:0;box-shadow:none}.chat-header{padding:15px 14px}.chat-messages{min-height:calc(100vh - 200px);max-height:none;padding:20px 14px}.message{max-width:94%;font-size:14px}.composer{padding:14px}.composer-row{gap:8px}.send-btn{min-width:78px;font-size:11px}.quick-prompts{display:none}.chat-footnote{font-size:9px}.chat-meta select{width:120px;font-size:10px}.badge{font-size:9px}}
@media(max-width:720px){.sidebar{position:relative;width:100%;height:auto;padding:14px 16px;gap:12px;display:block}.sidebar-brand{display:inline-block;border:0;padding:0;font-size:17px}.sidebar-brand small{display:inline;margin-left:8px}.side-nav{display:flex;flex-direction:row;overflow:auto;margin-top:10px}.side-nav button{white-space:nowrap;padding:8px 10px;font-size:10px}.side-status{display:none}.shell{margin-left:0;border:0}.alert,.mast,.wrap,.footer{padding-left:20px;padding-right:20px}}.dark .control,.dark input,.dark select,.dark textarea,.dark .mode,.dark .btn,.dark .card,.dark .check,.dark .settings-box{background:#1f1f1f;color:var(--ink)}.dark .console{background:#000}.dark .alert{color:#090909}
...
<body>
<div class="shell">
  <aside id="sidebar" class="sidebar"><div class="sidebar-brand">GENAGENT<small>LOCAL CONTROL</small></div><nav class="side-nav"><button class="active" onclick="jumpTo('task')">▣ TASK</button><button onclick="jumpTo('runtime')">◫ RUNTIME</button><button onclick="jumpTo('workspace')">□ FILES</button><button onclick="openSettings()">⚙ SETTINGS</button></nav><div class="side-status">SYSTEM STATUS<strong id="side-ready">● CHECKING</strong><span>SECURE LOCAL SESSION</span></div></aside>
  ...
  <main class="wrap">
    <section id="task" class="chat-panel">
      <div class="chat-header"><div class="chat-title-row"><button class="menu-toggle" onclick="toggleSidebar()" aria-label="Open menu">☰</button><div><div class="chat-title">AI WORKSPACE</div><span class="chat-subtitle">DIRECT CONVERSATION · LOCAL AGENT · BOUNDED ACTIONS</span></div></div><div class="chat-meta"><span id="ready" class="badge">AGENT: CHECKING</span><select id="model" aria-label="Model"><option value="gemini-3.6-flash">GEMINI FLASH</option><option value="gemini-flash-lite-latest">GEMINI LITE</option></select></div></div>
      <div id="console" class="chat-messages"><div class="message system"><span class="message-label">GENAGENT / SYSTEM</span>Hello. I am ready to help with your workspace. Ask me to inspect files, plan a change, or run an authorized task.</div></div>
      <div class="composer"><div class="composer-row"><textarea id="task-input" rows="2" placeholder="Message GenAgent..." onkeydown="if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();runTask()}"></textarea><button class="btn primary send-btn" id="run" onclick="runTask()">SEND ↗</button></div><div class="quick-prompts"><button onclick="usePrompt('Inspect this workspace and summarize the project.')">SUMMARIZE WORKSPACE</button><button onclick="usePrompt('Check the project for errors and suggest safe fixes.')">CHECK FOR ERRORS</button><button onclick="usePrompt('Show me the current project structure.')">PROJECT STRUCTURE</button></div><div class="chat-footnote">ENTER TO SEND · SHIFT+ENTER FOR NEW LINE · SUPERVISED MODE</div></div>
    </section>
    <section id="runtime" class="lower"><div class="card"><h2>RUNTIME STATUS</h2><div class="stat"><span>MODEL</span><b id="stat-model">—</b></div><div class="stat"><span>PLATFORM</span><b id="stat-platform">—</b></div><div class="stat"><span>WORKSPACE</span><b id="stat-workspace">—</b></div><div class="stat"><span>BUSY</span><b id="stat-busy">NO</b></div></div><div id="workspace" class="card"><h2>WORKSPACE FILES</h2><div id="files" class="file-list">Loading files...</div><button class="btn" style="margin-top:15px;width:100%" onclick="loadFiles()">REFRESH FILES</button></div></section>
  </main>
...
function appendMessage(role,text){const box=$('console');const item=document.createElement('div');item.className='message '+role;item.innerHTML='<span class="message-label">'+(role==='user'?'YOU':'GENAGENT')+'</span>'+escapeHtml(text);box.appendChild(item);box.scrollTop=box.scrollHeight;return item}
function usePrompt(text){$('task-input').value=text;$('task-input').focus()}
async function runTask(){const input=$('task-input');const task=input.value.trim();if(!task)return toast('Write a message first.');$('run').disabled=true;appendMessage('user',task);input.value='';const pending=appendMessage('agent','Thinking...');let streamed='';try{const d=await api('/api/run',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({task})});eventSource=new EventSource('/api/stream?task_id='+encodeURIComponent(d.task_id));eventSource.onmessage=e=>{const msg=JSON.parse(e.data);if(msg.type==='done'){pending.innerHTML='<span class="message-label">GENAGENT</span>'+escapeHtml(msg.response||streamed||'Task complete.');eventSource.close();$('run').disabled=false;loadStatus()}else{streamed+=(msg.text||'')+'\n';pending.innerHTML='<span class="message-label">GENAGENT / LIVE</span>'+escapeHtml(streamed);$('console').scrollTop=$('console').scrollHeight}};eventSource.onerror=()=>{pending.innerHTML='<span class="message-label">GENAGENT</span>'+escapeHtml(streamed||'Connection closed.');eventSource.close();$('run').disabled=false;loadStatus()}}catch(e){pending.innerHTML='<span class="message-label">GENAGENT / ERROR</span>'+escapeHtml(e.message);$('run').disabled=false}}
function clearTask(){$('task-input').value='';$('console').innerHTML='<div class="message system"><span class="message-label">GENAGENT / SYSTEM</span>Chat cleared. Ready for your next task.</div>'}
